...
class de_DE {
	...
	public function onLoad() {
		...
		add_filter( 'sanitize_title', array( $this, 'sanitizeTitle' ), 10, 2 );
		add_filter( 'sanitize_file_name', array( $this, 'normalize' ) );
		$this->onXmlrpc();
		$this->setRssLanguage();
	}

	public function onRest() {
		add_filter( 'wp_handle_upload_prefilter', array( $this, 'sanitizeUpload' ), 10, 1 );
	}
	...
	public function onXmlrpc() {
		if ( defined( 'XMLRPC_REQUEST' ) && XMLRPC_REQUEST ) {
			foreach ( $this->xmlrpcActions() as $action ) {
				add_action( $action, array( $this, 'sanitizeXmlrpContent' ) );
			}
		}
	}
